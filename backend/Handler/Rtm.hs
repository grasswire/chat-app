    ...
    Just slug -> do
      ...
      users <- case maybeChan of
                Just channel -> do
                  ...
                  let fiveMinsAgo = addUTCTime ((negate 5) :: NominalDiffTime) timeNow
...
usersPresentQuery chanKey lastseen = E.select $
                                     ...
                                     E.where_ $ E.exists $
                                                E.from $ \heartbeat -> do
                                                E.where_ (heartbeat E.^. HeartbeatChannel E.==. (E.val chanKey) E.&&. heartbeat E.^. HeartbeatUser E.==. user E.^. UserId E.&&. heartbeat E.^. HeartbeatLastSeen E.>=. (E.val lastseen))
                                     return user
