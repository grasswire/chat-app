...
messageFromEntity msg chanSlug likes = TP.Message (TP.UserId $ fromSqlKey (Model.messageUser msg)) (TP.MessageText $ Model.messageText msg) (Model.messageTimestamp msg) Nothing (TP.unChannelSlug chanSlug) (Schema._messageUuid $ Model.messageUuid msg) (likeFromEntity <$> likes)

likeFromEntity :: Model.MessageLike -> TP.MessageLike 
likeFromEntity like = TP.MessageLike (TP.MessageId $ Schema._messageUuid $ Model.messageLikeMessage like) (TP.UserId $ fromSqlKey (Model.messageLikeUser like)) (TP.ChannelSlug $ Schema.unSlug $ Model.messageLikeChannel like) (Model.messageLikeTimestamp like)
